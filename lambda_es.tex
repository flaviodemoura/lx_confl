...
In what follows, we will adopt a mixed-notation approach, intertwining metanotation with the equivalent Coq notation. This strategy aids in elucidating the proof steps of the upcoming lemmas, enabling a clearer and more detailed comprehension of each stage in the argumentation. The corresponding Coq code for the swapping of variables, named \coqdocvar{vswap}, is defined as follows: 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
\coqdockw{Definition} \coqdef{lambda es.vswap}{vswap}{\coqdocdefinition{vswap}} (\coqdef{lambda es.x:27}{x}{\coqdocbinder{x}}:\coqdocabbreviation{atom}) (\coqdef{lambda es.y:28}{y}{\coqdocbinder{y}}:\coqdocabbreviation{atom}) (\coqdef{lambda es.z:29}{z}{\coqdocbinder{z}}:\coqdocabbreviation{atom}) := \coqdockw{if} (\coqref{lambda es.z:29}{\coqdocvariable{z}} \coqdocnotation{==} \coqref{lambda es.x:27}{\coqdocvariable{x}}) \coqdockw{then} \coqref{lambda es.y:28}{\coqdocvariable{y}} \coqdockw{else} \coqdockw{if} (\coqref{lambda es.z:29}{\coqdocvariable{z}} \coqdocnotation{==} \coqref{lambda es.y:28}{\coqdocvariable{y}}) \coqdockw{then} \coqref{lambda es.x:27}{\coqdocvariable{x}} \coqdockw{else} \coqref{lambda es.z:29}{\coqdocvariable{z}}.\coqdoceol
\coqdocemptyline
\end{coqdoccode}
\noindent therefore, the swap $\vswap{x}{y}{z}$ is written in Coq as \coqdocvar{vswap} \coqdocvar{x} \coqdocvar{y} \coqdocvar{z}. In a short example to acquaint ourselves with the Coq notation, let us show how we will write the proofs:
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.vswap id}{vswap\_id}{\coqdoclemma{vswap\_id}}: \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.x:30}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:31}{y}{\coqdocbinder{y}}, \coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:30}{\coqdocvariable{x}} \coqref{lambda es.x:30}{\coqdocvariable{x}} \coqref{lambda es.y:31}{\coqdocvariable{y}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.y:31}{\coqdocvariable{y}}.\coqdoceol
\end{coqdoccode}
...
\coqdocnoindent
\coqdockw{Notation} \coqdef{lambda es.:::'[' x ':=' x ']' x}{"}{"}[ x := u ] t" := (\coqref{lambda es.n sub}{\coqdocconstructor{n\_sub}} \coqdocvar{t} \coqdocvar{x} \coqdocvar{u}) (\coqdoctac{at} \coqdockw{level} 60).\coqdoceol
\coqdocemptyline
\end{coqdoccode}
...
\coqdocindent{1.00em}
\ensuremath{|} \coqref{lambda es.n var}{\coqdocconstructor{n\_var}} \coqdocvar{z}     \ensuremath{\Rightarrow} \coqref{lambda es.n var}{\coqdocconstructor{n\_var}} (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{z})\coqdoceol
\coqdocindent{1.00em}
\ensuremath{|} \coqref{lambda es.n abs}{\coqdocconstructor{n\_abs}} \coqdocvar{z} \coqdocvar{t1}  \ensuremath{\Rightarrow} \coqref{lambda es.n abs}{\coqdocconstructor{n\_abs}} (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{z}) (\coqref{lambda es.swap:51}{\coqdocdefinition{swap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{t1})\coqdoceol
\coqdocindent{1.00em}
...
\coqdocindent{1.00em}
\ensuremath{|} \coqref{lambda es.n sub}{\coqdocconstructor{n\_sub}} \coqdocvar{t1} \coqdocvar{z} \coqdocvar{t2} \ensuremath{\Rightarrow} \coqref{lambda es.n sub}{\coqdocconstructor{n\_sub}} (\coqref{lambda es.swap:51}{\coqdocdefinition{swap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{t1}) (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{z}) (\coqref{lambda es.swap:51}{\coqdocdefinition{swap}} \coqref{lambda es.x:48}{\coqdocvariable{x}} \coqref{lambda es.y:49}{\coqdocvariable{y}} \coqdocvar{t2})\coqdoceol
\coqdocindent{1.00em}
\coqdockw{end}.\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
The \coqdocvar{swap} function has many interesting properties, but we will focus on the ones that are more relevant to the proofs related to the substitution lemma. Nevertheless, all lemmas can be found in the source code of the formalization \cite{msubstlemma}. The next lemma shows that the \coqdocvar{swap} function preserves the size of terms. It is proved by induction on the structure of \coqdocvar{t}: 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.swap size eq}{swap\_size\_eq}{\coqdoclemma{swap\_size\_eq}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.x:67}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:68}{y}{\coqdocbinder{y}} \coqdef{lambda es.t:69}{t}{\coqdocbinder{t}}, \coqref{lambda es.size}{\coqdocdefinition{size}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:67}{\coqdocvariable{x}} \coqref{lambda es.y:68}{\coqdocvariable{y}} \coqref{lambda es.t:69}{\coqdocvariable{t}}) \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.size}{\coqdocdefinition{size}} \coqref{lambda es.t:69}{\coqdocvariable{t}}.\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
The \coqdocvar{swap} function is involutive, which is also proved by structural induction on \coqdocvar{t}: 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.swap involutive}{swap\_involutive}{\coqdoclemma{swap\_involutive}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:78}{t}{\coqdocbinder{t}} \coqdef{lambda es.x:79}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:80}{y}{\coqdocbinder{y}}, \coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:79}{\coqdocvariable{x}} \coqref{lambda es.y:80}{\coqdocvariable{y}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:79}{\coqdocvariable{x}} \coqref{lambda es.y:80}{\coqdocvariable{y}} \coqref{lambda es.t:78}{\coqdocvariable{t}}) \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.t:78}{\coqdocvariable{t}}.\coqdoceol
\coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.shuffle swap}{shuffle\_swap}{\coqdoclemma{shuffle\_swap}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.w:81}{w}{\coqdocbinder{w}} \coqdef{lambda es.y:82}{y}{\coqdocbinder{y}} \coqdef{lambda es.z:83}{z}{\coqdocbinder{z}} \coqdef{lambda es.t:84}{t}{\coqdocbinder{t}}, \coqref{lambda es.w:81}{\coqdocvariable{w}} \coqexternalref{::type scope:x '<>' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\not=}}} \coqref{lambda es.z:83}{\coqdocvariable{z}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.y:82}{\coqdocvariable{y}} \coqexternalref{::type scope:x '<>' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\not=}}} \coqref{lambda es.z:83}{\coqdocvariable{z}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.w:81}{\coqdocvariable{w}} \coqref{lambda es.y:82}{\coqdocvariable{y}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.y:82}{\coqdocvariable{y}} \coqref{lambda es.z:83}{\coqdocvariable{z}} \coqref{lambda es.t:84}{\coqdocvariable{t}})\coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.w:81}{\coqdocvariable{w}} \coqref{lambda es.z:83}{\coqdocvariable{z}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.w:81}{\coqdocvariable{w}} \coqref{lambda es.y:82}{\coqdocvariable{y}} \coqref{lambda es.t:84}{\coqdocvariable{t}})\coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}}.\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.vswap equivariance}{vswap\_equivariance}{\coqdoclemma{vswap\_equivariance}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.v:89}{v}{\coqdocbinder{v}} \coqdef{lambda es.x:90}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:91}{y}{\coqdocbinder{y}} \coqdef{lambda es.z:92}{z}{\coqdocbinder{z}} \coqdef{lambda es.w:93}{w}{\coqdocbinder{w}}, \coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:90}{\coqdocvariable{x}} \coqref{lambda es.y:91}{\coqdocvariable{y}} (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.z:92}{\coqdocvariable{z}} \coqref{lambda es.w:93}{\coqdocvariable{w}} \coqref{lambda es.v:89}{\coqdocvariable{v}}) \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.vswap}{\coqdocdefinition{vswap}} (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:90}{\coqdocvariable{x}} \coqref{lambda es.y:91}{\coqdocvariable{y}} \coqref{lambda es.z:92}{\coqdocvariable{z}}) (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:90}{\coqdocvariable{x}} \coqref{lambda es.y:91}{\coqdocvariable{y}} \coqref{lambda es.w:93}{\coqdocvariable{w}}) (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:90}{\coqdocvariable{x}} \coqref{lambda es.y:91}{\coqdocvariable{y}} \coqref{lambda es.v:89}{\coqdocvariable{v}}).\coqdoceol
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.swap equivariance}{swap\_equivariance}{\coqdoclemma{swap\_equivariance}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:94}{t}{\coqdocbinder{t}} \coqdef{lambda es.x:95}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:96}{y}{\coqdocbinder{y}} \coqdef{lambda es.z:97}{z}{\coqdocbinder{z}} \coqdef{lambda es.w:98}{w}{\coqdocbinder{w}}, \coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:95}{\coqdocvariable{x}} \coqref{lambda es.y:96}{\coqdocvariable{y}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.z:97}{\coqdocvariable{z}} \coqref{lambda es.w:98}{\coqdocvariable{w}} \coqref{lambda es.t:94}{\coqdocvariable{t}}) \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.swap}{\coqdocdefinition{swap}} (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:95}{\coqdocvariable{x}} \coqref{lambda es.y:96}{\coqdocvariable{y}} \coqref{lambda es.z:97}{\coqdocvariable{z}}) (\coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:95}{\coqdocvariable{x}} \coqref{lambda es.y:96}{\coqdocvariable{y}} \coqref{lambda es.w:98}{\coqdocvariable{w}}) (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:95}{\coqdocvariable{x}} \coqref{lambda es.y:96}{\coqdocvariable{y}} \coqref{lambda es.t:94}{\coqdocvariable{t}}).\coqdoceol
\coqdocemptyline
\end{coqdoccode}
If a variable, say \coqdocvar{z}, is not in the set of free variables of a term \coqdocvar{t} and one swaps \coqdocvar{z} with another variable, say \coqdocvar{y}, then \coqdocvar{y} is not in the set of free variables of the term \coqdocvar{t}. This is the content of the following lemma that can easily be proved using induction on the structure of \coqdocvar{t}:
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.fv nom swap}{fv\_nom\_swap}{\coqdoclemma{fv\_nom\_swap}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.z:99}{z}{\coqdocbinder{z}} \coqdef{lambda es.y:100}{y}{\coqdocbinder{y}} \coqdef{lambda es.t:101}{t}{\coqdocbinder{t}}, \coqref{lambda es.z:99}{\coqdocvariable{z}} \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} \coqref{lambda es.t:101}{\coqdocvariable{t}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.y:100}{\coqdocvariable{y}} \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.y:100}{\coqdocvariable{y}} \coqref{lambda es.z:99}{\coqdocvariable{z}} \coqref{lambda es.t:101}{\coqdocvariable{t}}).\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.n sexp induction}{n\_sexp\_induction}{\coqdoclemma{n\_sexp\_induction}}: \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.P:113}{P}{\coqdocbinder{P}} : \coqref{lambda es.n sexp}{\coqdocinductive{n\_sexp}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqdockw{Prop}, \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.x:114}{x}{\coqdocbinder{x}}, \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.n var}{\coqdocconstructor{n\_var}} \coqref{lambda es.x:114}{\coqdocvariable{x}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}}\coqdoceol
\coqdocindent{0.50em}
\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t1:115}{t1}{\coqdocbinder{t1}} \coqdef{lambda es.z:116}{z}{\coqdocbinder{z}}, \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t2:117}{t2}{\coqdocbinder{t2}} \coqdef{lambda es.x:118}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:119}{y}{\coqdocbinder{y}}, \coqref{lambda es.size}{\coqdocdefinition{size}} \coqref{lambda es.t2:117}{\coqdocvariable{t2}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.size}{\coqdocdefinition{size}} \coqref{lambda es.t1:115}{\coqdocvariable{t1}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:118}{\coqdocvariable{x}} \coqref{lambda es.y:119}{\coqdocvariable{y}} \coqref{lambda es.t2:117}{\coqdocvariable{t2}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.n abs}{\coqdocconstructor{n\_abs}} \coqref{lambda es.z:116}{\coqdocvariable{z}} \coqref{lambda es.t1:115}{\coqdocvariable{t1}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}}\coqdoceol
\coqdocindent{0.50em}
\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t1:120}{t1}{\coqdocbinder{t1}} \coqdef{lambda es.t2:121}{t2}{\coqdocbinder{t2}}, \coqref{lambda es.P:113}{\coqdocvariable{P}} \coqref{lambda es.t1:120}{\coqdocvariable{t1}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} \coqref{lambda es.t2:121}{\coqdocvariable{t2}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.n app}{\coqdocconstructor{n\_app}} \coqref{lambda es.t1:120}{\coqdocvariable{t1}} \coqref{lambda es.t2:121}{\coqdocvariable{t2}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}}\coqdoceol
\coqdocindent{0.50em}
\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t1:122}{t1}{\coqdocbinder{t1}} \coqdef{lambda es.t3:123}{t3}{\coqdocbinder{t3}} \coqdef{lambda es.z:124}{z}{\coqdocbinder{z}}, \coqref{lambda es.P:113}{\coqdocvariable{P}} \coqref{lambda es.t3:123}{\coqdocvariable{t3}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t2:125}{t2}{\coqdocbinder{t2}} \coqdef{lambda es.x:126}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:127}{y}{\coqdocbinder{y}}, \coqref{lambda es.size}{\coqdocdefinition{size}} \coqref{lambda es.t2:125}{\coqdocvariable{t2}} \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.size}{\coqdocdefinition{size}} \coqref{lambda es.t1:122}{\coqdocvariable{t1}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:126}{\coqdocvariable{x}} \coqref{lambda es.y:127}{\coqdocvariable{y}} \coqref{lambda es.t2:125}{\coqdocvariable{t2}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.P:113}{\coqdocvariable{P}} (\coqref{lambda es.n sub}{\coqdocconstructor{n\_sub}} \coqref{lambda es.t1:122}{\coqdocvariable{t1}} \coqref{lambda es.z:124}{\coqdocvariable{z}} \coqref{lambda es.t3:123}{\coqdocvariable{t3}})\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{(}}\coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:128}{t}{\coqdocbinder{t}}, \coqref{lambda es.P:113}{\coqdocvariable{P}} \coqref{lambda es.t:128}{\coqdocvariable{t}}\coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{)}}.\coqdoceol
\coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.notin fv nom equivariance}{notin\_fv\_nom\_equivariance}{\coqdoclemma{notin\_fv\_nom\_equivariance}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:129}{t}{\coqdocbinder{t}} \coqdef{lambda es.x':130}{x'}{\coqdocbinder{x'}} \coqdef{lambda es.x:131}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:132}{y}{\coqdocbinder{y}}, \coqref{lambda es.x':130}{\coqdocvariable{x'}} \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} \coqref{lambda es.t:129}{\coqdocvariable{t}} \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:131}{\coqdocvariable{x}} \coqref{lambda es.y:132}{\coqdocvariable{y}} \coqref{lambda es.x':130}{\coqdocvariable{x'}}  \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:131}{\coqdocvariable{x}} \coqref{lambda es.y:132}{\coqdocvariable{y}} \coqref{lambda es.t:129}{\coqdocvariable{t}}).\coqdoceol
\end{coqdoccode}
\noindent{\bf Proof.} The proof is by induction on the size of the term \coqdocvar{t}.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{t} is a variable, say \coqdocvar{z}, then we have that \coqdocvar{x'} \ensuremath{\not=} \coqdocvar{z} by hypothesis, and we conclude by lemma \coqdocvar{swap\_neq} \cite{msubstlemma}.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{t} = \coqdocvar{n\_abs} \coqdocvar{z} \coqdocvar{t1} then we have that $x' \notin fv(t1) \backslash \{z\}$ by hypothesis. This means that either \coqdocvar{x'} = \coqdocvar{z} or \coqdocvar{x'} is not in $fv(t1)$, {\it i.e.} \coqdocvar{fv\_nom} \coqdocvar{t1} in Coq.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{x'} = \coqdocvar{z} then we have to prove that a certain element is not in a set where it has been removed, and we are done by lemma \coqdocvar{notin\_remove\_3}\footnote{This is a lemma from Metalib library and it states that {\tt forall (x y : atom) (s : atoms), x = y -> y `notin` remove x s}.}. 
\begin{coqdoccode}
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.notin fv nom remove swap}{notin\_fv\_nom\_remove\_swap}{\coqdoclemma{notin\_fv\_nom\_remove\_swap}}: \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:133}{t}{\coqdocbinder{t}} \coqdef{lambda es.x':134}{x'}{\coqdocbinder{x'}} \coqdef{lambda es.x:135}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:136}{y}{\coqdocbinder{y}}, \coqref{lambda es.vswap}{\coqdocdefinition{vswap}} \coqref{lambda es.x:135}{\coqdocvariable{x}} \coqref{lambda es.y:136}{\coqdocvariable{y}} \coqref{lambda es.x':134}{\coqdocvariable{x'}} \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:135}{\coqdocvariable{x}} \coqref{lambda es.y:136}{\coqdocvariable{y}} \coqref{lambda es.t:133}{\coqdocvariable{t}}) \coqexternalref{::type scope:x '->' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{\ensuremath{\rightarrow}}} \coqref{lambda es.x':134}{\coqdocvariable{x'}} \coqdocnotation{`}\coqdocnotation{notin}\coqdocnotation{`} \coqref{lambda es.fv nom}{\coqdocdefinition{fv\_nom}} \coqref{lambda es.t:133}{\coqdocvariable{t}}.\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
\end{coqdoccode}
\noindent{\bf Proof.} Initially, observe the similarity of the left hand side (LHS) of the $\alpha$-equation with the lemma \coqdocvar{shuffle\_swap}. In order to use it, we need to have that both \coqdocvar{z} \ensuremath{\not=} \coqdocvar{y} and \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y}. 
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{z} = \coqdocvar{y} then the right hand side (RHS) reduces to \coqdocvar{t} because the swap is trivial, and the LHS also reduces to \coqdocvar{t} since swap is involutive.
\begin{coqdoccode}
\end{coqdoccode}
...
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{y} = \coqdocvar{x} then \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y0}, where y0 is the new variable generated by applying the definition of the meta-substitution, and the metasubstitution [\coqdocvar{x} := \coqdocvar{u}] has no effect on the LHS, but it can be propagated on the RHS, {\it i.e.} over the abstraction (\coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2}) but it also has no effect in \coqdocvar{t2} because \coqdocvar{x} does not occur free in \coqdocvar{t2}. 
\begin{coqdoccode}
\end{coqdoccode}
...
\coqdocemptyline
\end{coqdoccode}
Now, we show how to propagate a swap inside metasubstitutions using the decomposition of the metasubstitution provided by the corollary \coqdocvar{aeq\_m\_subst\_eq}.\newline 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\coqdocemptyline
\end{coqdoccode}
In fact, the need of the lemma \coqdocvar{swap\_subst\_rec\_fun} in the proofs of the two previous lemmas is justified by the fact that when the $\alpha$-equation involves abstractions with different binders, or explicit substitutions with different binders, the rules \coqdocvar{aeq\_abs\_diff} and \coqdocvar{aeq\_sub\_diff} introduce swaps that are outside the metasubstitutions. 
\begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
\section{The substitution lemma}
 ...
 In the pure $\lambda$-calculus, the substitution lemma is probably the first non trivial property. In our framework, we have defined two different substitution operators, namely, the metasubstitution denoted by \{\coqdocvar{x}:=\coqdocvar{u}\}\coqdocvar{t} and the explicit substitution, written as [\coqdocvar{x}:=\coqdocvar{u}]\coqdocvar{t}. In what follows, we present the main steps of our proof of the substitution lemma for \coqdocvar{n\_sexp} terms, {\it i.e.} for nominal terms with explicit substitutions. 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\end{coqdoccode}
\noindent{\bf Proof.} The proof is by induction on the size of \coqdocvar{t1}. The interesting cases are the abstraction and the explicit substitution.
\begin{coqdoccode}
\end{coqdoccode}
