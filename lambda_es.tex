...
\coqdocemptyline
\end{coqdoccode}
\noindent where \coqdocvar{n\_var} is the constructor for variables, \coqdocvar{n\_abs} for abstractions, \coqdocvar{n\_app} for applications and \coqdocvar{n\_sub} for the explicit substitution. The \coqdocvar{size} and the set \coqdocvar{fv\_nom} of the free variables of a term are defined as usual: 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdef{lambda es.swap involutive}{swap\_involutive}{\coqdoclemma{swap\_involutive}} : \coqdockw{\ensuremath{\forall}} \coqdef{lambda es.t:74}{t}{\coqdocbinder{t}} \coqdef{lambda es.x:75}{x}{\coqdocbinder{x}} \coqdef{lambda es.y:76}{y}{\coqdocbinder{y}},\coqdoceol
\coqdocindent{2.00em}
\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:75}{\coqdocvariable{x}} \coqref{lambda es.y:76}{\coqdocvariable{y}} (\coqref{lambda es.swap}{\coqdocdefinition{swap}} \coqref{lambda es.x:75}{\coqdocvariable{x}} \coqref{lambda es.y:76}{\coqdocvariable{y}} \coqref{lambda es.t:74}{\coqdocvariable{t}}) \coqexternalref{::type scope:x '=' x}{http://coq.inria.fr/distrib/V8.16.1/stdlib//Coq.Init.Logic}{\coqdocnotation{=}} \coqref{lambda es.t:74}{\coqdocvariable{t}}.\coqdoceol
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
Equivariance is an important property of the \coqdocvar{swap} function. It states that a swap can uniformly be propagated over the structure of a term:
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
