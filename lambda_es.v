    ...
    + intro Hneq'. case (z == y). 
      * intro Heq. subst. pose proof m_subst_sub_neq as Hsubneq. pick fresh w for (union (fv_nom e1_1) (union (fv_nom e1_2) (union (fv_nom e2) (union (fv_nom e3) (union (singleton x) (singleton y)))))).
        ...
        ** apply aeq_trans with ([y := e3] n_sub ([x := e2] swap y w e1_1) w ([x := e2] e1_2)).
           ...
           *** pose proof m_subst_sub_neq as Hsubneq'. pick fresh w' for (union (fv_nom e1_1) (union (fv_nom e1_2) (union (fv_nom e2) (union (fv_nom e3) (union (singleton x) (union (singleton w) (singleton y))))))).
               specialize (Hsubneq' ([x := e2] swap y w e1_1) ([x := e2] e1_2) e3 y w w'). apply aeq_trans with (([y := e3] n_sub ([x := e2] swap y w e1_1) w ([x := e2] e1_2))). rewrite Hsubneq'.
               **** apply aeq_refl.
               **** admit.
               **** admit.
               **** rewrite Hsubneq'.
                    ***** pose proof m_subst_sub_eq as Hsubeq. specialize (Hsubeq e3 y e1_1 e1_2). rewrite Hsubeq.
                          pose proof m_subst_sub_neq as Hsubneq''. specialize (Hsubneq'' e1_1 ([y := e3] e1_2) ([y := e3] e2) x y w). rewrite Hsubneq''.
                          ****** apply aeq_sub_diff.
                                 ******* apply IHe1_1.
                                         ******** assumption.
                                         ******** assumption.
                                 ******* admit.
                                 ******* admit.
                                 ******* pose proof swap_subst_rec_fun as Hswap. unfold m_subst in *. specialize (Hswap w w' x (swap y w e1_1) ([y := e3] e2)).
                                         rewrite Hswap. unfold swap_var. destruct (x == w).
                                         ******** subst. unfold m_subst. pose proof swap_subst_rec_fun as Hswap'. specialize (Hswap' w w' y e2 e3).
                                                  ********* apply aeq_trans with (subst_rec_fun (swap w w' (swap y w e1_1)) (subst_rec_fun (swap w w' e2) (swap w w' e3) (swap_var w w' y)) w').
                                                            ********** unfold swap_var. destruct (y == w).
                                                                       *********** subst. contradiction.
                                                                       *********** destruct (y == w'). subst.
                                                                                   ************ admit.
