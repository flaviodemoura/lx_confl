...
(** %\noindent% where [n_var] is the constructor for variables, [n_abs] for abstractions, [n_app] for applications and [n_sub] for the explicit substitution. The [size] and the set [fv_nom] of the free variables of a term are defined as usual: *)
...
Lemma notin_fv_nom_equivariance : forall x0 x y t ,
  x0 `notin` fv_nom t ->
  vswap x y x0  `notin` fv_nom (swap x y t).
Proof.
  intros. unfold vswap. case (x0 == x).
