...
\end{coqdoccode}
\noindent where (\coqdocvar{n\_sub} \coqdocvar{t1} \coqdocvar{x} \coqdocvar{t2}) is written [\coqdocvar{x} := \coqdocvar{t2}]\coqdocvar{t1} from now on. The \coqdocvar{size} and the set \coqdocvar{fv\_nom} of the free variables of a term are defined as usual: 
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\end{coqdoccode}
\noindent{\bf Proof.} The proof is by induction on the size of the term \coqdocvar{t}. The abstraction and the explicit substitution are the interesting cases.
\begin{coqdoccode}
\end{coqdoccode}
In the former case, we need to prove that (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1}) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{t'}), where \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t'} by hypothesis.\begin{coqdoccode}
 \end{coqdoccode}
Therefore, \coqdocvar{t'} is an abstraction, and according to our definition of $\alpha$-equivalence there are two subcases: \newline {\bf 1.} In the first subcase, \coqdocvar{t'} also has \coqdocvar{y} as binding variable {\it i.e.} \coqdocvar{t'} = \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t2}, where \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t2}, and hence the current goal is given by (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1}) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t2}).
\begin{coqdoccode}
\end{coqdoccode}
We proceed by comparing \coqdocvar{x} and \coqdocvar{y}.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{x} = \coqdocvar{y} then, we are done by using twice lemma \coqdocvar{m\_subst\_abs\_eq}.
\begin{coqdoccode}
\end{coqdoccode}
When \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y}, then we need to propagate the metasubstitution on both sides of the $\alpha$-equation.\begin{coqdoccode}
 \end{coqdoccode}
On the LHS, we need a fresh name that is not in the set $fv(u)\cup fv(\lambda_y.t_1) \cup \{x\}$, while for the RHS, the fresh name cannot belong to the set $fv(u)\cup fv(\lambda_y.t_1) \cup \{x\}$.\begin{coqdoccode}
 \end{coqdoccode}
From the hypothesis \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t2}, we get that $fv(t_1) = fv(t_2)$ using lemma \coqdocvar{aeq\_fv\_nom} and the axiom \coqdocvar{Eq\_implies\_equality}\begin{coqdoccode}
 \end{coqdoccode}
Therefore, using this equality the sets become equal, and we can generate only one name, say \coqdocvar{x0}, satisfying the conditions for both LHS and RHS of the $\alpha$-equation. This means that the current goal has the same binding variable on both sides of the $\alpha$-equation. We proceed using the constructor \coqdocvar{aeq\_abs\_same}, and conclude by induction hypothesis.\begin{coqdoccode}
 \end{coqdoccode}
\newline {\bf 2.} In the second subcase, \coqdocvar{t'} = \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2}, where \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{swap} \coqdocvar{y0} \coqdocvar{y} \coqdocvar{t2} and \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0}. The current goal is (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1}) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2}), and we proceed by comparing \coqdocvar{x} and \coqdocvar{y} in the LHS:
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{y} = \coqdocvar{x} then the metasubstitution \{\coqdocvar{x} := \coqdocvar{u}\} has no effect on the LHS, but in this case \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0} and in the RHS the metasubstitution has to be propagated.\begin{coqdoccode}
 \end{coqdoccode}
Let \coqdocvar{x} be a fresh name not in the set $fv(u)\cup fv(\lambda_{y_0}.t_2)\cup \{y\}$. The current goal is \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{n\_abs} \coqdocvar{x} \{\coqdocvar{y} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y0} \coqdocvar{x} \coqdocvar{t2}), but the metasubstitution \{\coqdocvar{y} := \coqdocvar{u}\} has no effect in the term (\coqdocvar{swap} \coqdocvar{y0} \coqdocvar{x} \coqdocvar{t2}) because \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0}, \coqdocvar{y} \ensuremath{\not=} \coqdocvar{x} and \coqdocvar{y} does not occur free in \coqdocvar{t2} by hypothesis.\begin{coqdoccode}
 \end{coqdoccode}
The proof that \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{n\_abs} \coqdocvar{x} (\coqdocvar{swap} \coqdocvar{y0} \coqdocvar{x} \coqdocvar{t2}) is straightforward because \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2} by hypothesis.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{y} \ensuremath{\not=} \coqdocvar{x} then we proceed by comparing \coqdocvar{x} and \coqdocvar{y0} on the RHS, and the proof when \coqdocvar{x} = \coqdocvar{y0} is analogous to the previous subcase.\begin{coqdoccode}
 \end{coqdoccode}
When both \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y} and \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y0} then we need to propagate the metasubstitution on both sides of the current goal: (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1}) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2}).\begin{coqdoccode}
 \end{coqdoccode}
By hypothesis, \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2} and hence the set of free variables of \coqdocvar{n\_abs} \coqdocvar{y} \coqdocvar{t1} is equal to the set of free variables of \coqdocvar{n\_abs} \coqdocvar{y0} \coqdocvar{t2}. Therefore, only one fresh name, say \coqdocvar{x0}, that is not in the set $x_0 \notin fv(u) \cup fv(\lambda_{y_0}.t_2) \cup \{x\}$ is enough to fulfill the conditions for propagating the metasubstitutions on both sides of the $\alpha$-equation, and we are done by the induction hypothesis.\begin{coqdoccode}
 \end{coqdoccode}
The explicit substitution operation is also interesting. Initially, the goal is (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y} := \coqdocvar{t2}] \coqdocvar{t1})) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} \coqdocvar{t}), and according to the definition of $\alpha$-equivalence, there are 2 subcases.\begin{coqdoccode}
 \end{coqdoccode}
\newline {\bf 1.} In the first subcase, \coqdocvar{t} = ([\coqdocvar{y} := \coqdocvar{t2'}] \coqdocvar{t1'}) with \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t1'} and \coqdocvar{t2} =\coqdocvar{a} \coqdocvar{t2'}.
\begin{coqdoccode}
\end{coqdoccode}
As in the abstraction case, we start comparing \coqdocvar{x} and \coqdocvar{y}.
\begin{coqdoccode}
\end{coqdoccode}
When \coqdocvar{x} = \coqdocvar{y}, the proof is trivial because both metasubstitutions are removed by applying lemma \coqdocvar{m\_subst\_sub\_eq} twice, and we get the following goal: ([\coqdocvar{y} := \{\coqdocvar{y} := \coqdocvar{u}\} \coqdocvar{t2}] \coqdocvar{t1}) =\coqdocvar{a} ([\coqdocvar{y} := \{\coqdocvar{y} := \coqdocvar{u}\} \coqdocvar{t2'}] \coqdocvar{t1'})\begin{coqdoccode}
 \end{coqdoccode}
We compare the corresponding components of the explicit substitution via the constructor \coqdocvar{aeq\_sub\_same}, and the first case is trivial since \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t1'}.
\begin{coqdoccode}
\end{coqdoccode}
In order to show that (\{\coqdocvar{y} := \coqdocvar{u}\} \coqdocvar{t2}) =\coqdocvar{a} (\{\coqdocvar{y} := \coqdocvar{u}\} \coqdocvar{t2'}), we apply the induction hypothesis.
\begin{coqdoccode}
\end{coqdoccode}
When \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y}, we propagate the metasubstitutions inside the explicit substitution on both sides.\begin{coqdoccode}
 \end{coqdoccode}
As \coqdocvar{t1} =\coqdocvar{a} \coqdocvar{t1'} and \coqdocvar{t2} =\coqdocvar{a} \coqdocvar{t2'}, we have that \coqdocvar{fv\_nom} \coqdocvar{t1} = \coqdocvar{fv\_nom} \coqdocvar{t1'} and \coqdocvar{fv\_nom} \coqdocvar{t2} = \coqdocvar{fv\_nom} \coqdocvar{t2'}, and we need just one fresh name, say \coqdocvar{x0}, to do these propagations, as long as \coqdocvar{x0} does not belong to the set $fv(u)\cup fv(\metasub{t_1'}{y}{t_2'})\cup \{x\}$. The goal after the propagation is ([\coqdocvar{x0} := \{\coqdocvar{x} := \coqdocvar{u}\}\coqdocvar{t2'}] \{\coqdocvar{x} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y} \coqdocvar{x0} \coqdocvar{t1'})) =\coqdocvar{a} ([\coqdocvar{x0} := \{\coqdocvar{x} := \coqdocvar{u}\}\coqdocvar{t2}] \{\coqdocvar{x} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y} \coqdocvar{x0} \coqdocvar{t1})), and we proceed by a componentwise comparison via constructor \coqdocvar{aeq\_sub\_same}.\begin{coqdoccode}
 \end{coqdoccode}
Each subcase is proved by the induction hypothesis.
\begin{coqdoccode}
\end{coqdoccode}
\newline {\bf 2.} In the second subcase, the goal is (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y} := \coqdocvar{t2}] \coqdocvar{t1})) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y0} := \coqdocvar{t2'}] \coqdocvar{t1'})) with \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0}. We proceed by comparing \coqdocvar{x} and \coqdocvar{y}.
\begin{coqdoccode}
\end{coqdoccode}
If \coqdocvar{x} = \coqdocvar{y} then the metasubstitution of the LHS only propagates to the subterm \coqdocvar{t2}.\begin{coqdoccode}
 \end{coqdoccode}
In the RHS, the metasubstitution is propagated to both subterms because \coqdocvar{x} = \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0}.\begin{coqdoccode}
 \end{coqdoccode}
To do so, we take a fresh name \coqdocvar{x} that is not in the set $fv(u) \cup fv(\esub{t_1'}{y_0}{t_2'})$. We proceed by comparing componentwise according to the constructor \coqdocvar{aeq\_sub\_diff}.\begin{coqdoccode}
 \end{coqdoccode}
The proof that \{\coqdocvar{y} := \coqdocvar{u}\}\coqdocvar{t2} =\coqdocvar{a} \{\coqdocvar{y} := \coqdocvar{u}\}\coqdocvar{t2'} is straightforward by the induction hypothesis.
\begin{coqdoccode}
\end{coqdoccode}
The proof that \{\coqdocvar{y} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y0} \coqdocvar{x} \coqdocvar{t1'}) =\coqdocvar{a} \coqdocvar{swap} \coqdocvar{y} \coqdocvar{x} \coqdocvar{t1} is done by lemma \coqdocvar{m\_subst\_notin} since \coqdocvar{y} \ensuremath{\not=} \coqdocvar{y0}, \coqdocvar{y} \ensuremath{\not=} \coqdocvar{x} and \coqdocvar{y} is not in $fv(t_1')$.
\begin{coqdoccode}
\end{coqdoccode}
When \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y}, we start with the following goal: (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y} := \coqdocvar{t2}] \coqdocvar{t1})) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y0} := \coqdocvar{t2'}] \coqdocvar{t1'})). We proceed by comparing \coqdocvar{x} and \coqdocvar{y0}.\begin{coqdoccode}
 \end{coqdoccode}
When \coqdocvar{x} = \coqdocvar{y0}, the strategy is similar to the previous case \coqdocvar{x} = \coqdocvar{y} and \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y0}.\begin{coqdoccode}
 \end{coqdoccode}
The last case is when \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y} and \coqdocvar{x} \ensuremath{\not=} \coqdocvar{y0}. The current goal is (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y} := \coqdocvar{t2}] \coqdocvar{t1})) =\coqdocvar{a} (\{\coqdocvar{x} := \coqdocvar{u}\} ([\coqdocvar{y0} := \coqdocvar{t2'}] \coqdocvar{t1'})).\begin{coqdoccode}
 \end{coqdoccode}
We take a fresh name \coqdocvar{x0} that is not in the set $fv(u)\cup fv(\esub{t_1'}{y_0}{t_2'})\cup \{ x \}$, and propagate the metasubstitutions inside the explicit substitutions according to the definition of the metasubstitution. The current goal is ([\coqdocvar{x0} := \{\coqdocvar{x} := \coqdocvar{u}\}\coqdocvar{t2'}](\{\coqdocvar{x} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y0} \coqdocvar{x0} \coqdocvar{t1'}))) =\coqdocvar{a}
  ([\coqdocvar{x0} := \{\coqdocvar{x} := \coqdocvar{u}\}\coqdocvar{t2}](\{\coqdocvar{x} := \coqdocvar{u}\}(\coqdocvar{swap} \coqdocvar{y} \coqdocvar{x0} \coqdocvar{t1}))), and we proceed using the constructor \coqdocvar{aeq\_sub\_same}. Each subcase is proved by the induction hypothesis. $\hfill\Box$\begin{coqdoccode}
 \coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\end{coqdoccode}
The lemma \coqdocvar{m\_subst\_sub\_neq} that follows is the analogous version of the previous lemma for the case of the explicit substitution:
\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Lemma} \coqdocvar{fv\_nom\_metasub}: \coqdockw{\ensuremath{\forall}} \coqdocvar{t} \coqdocvar{u} \coqdocvar{x}, \coqdocvar{x} `\coqdocvar{notin}` (\coqdocvar{fv\_nom} \coqdocvar{t}) \ensuremath{\rightarrow}  \coqdocvar{fv\_nom} (\{\coqdocvar{x} := \coqdocvar{u}\}\coqdocvar{t}) [=] \coqdocvar{fv\_nom} \coqdocvar{t}.\coqdoceol
 \end{coqdoccode}
\noindent {\bf Proof.} The proof is by structural induction on \coqdocvar{t}.
\begin{coqdoccode}
\end{coqdoccode}
When \coqdocvar{t} is a variable, say \coqdocvar{y}, then both sides of the (set) equality reduce to $\{y\}$, and we are done.
\begin{coqdoccode}
\coqdocemptyline
\end{coqdoccode}
